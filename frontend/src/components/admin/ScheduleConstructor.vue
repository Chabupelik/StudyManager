...
async function loadSchedules() {
  ...
  loading.value = true;
  try {
    const res = await api.get(`/api/v1/schedule/admin/base?group_id=${props.groupId}`);
...
async function saveLesson() {
  ...
  try {
    ...
    if (editingLesson.value.id) {
      await api.put(`/api/v1/schedule/admin/base/lesson/${editingLesson.value.id}`, payload);
      uiStore.showToast('Пара обновлена', 'success');
    } else {
      await api.post(`/api/v1/schedule/admin/base/${props.groupId}/${editingLesson.value.day_of_week}`, payload);
...
async function deleteLesson(id: number) {
  ...
  try {
    await api.delete(`/api/v1/schedule/admin/base/lesson/${id}`);
